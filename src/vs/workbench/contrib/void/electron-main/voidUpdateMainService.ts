...
export class VoidMainUpdateService extends Disposable implements IVoidUpdateService {
	...
	async check(explicit: boolean): Promise<VoidCheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart veritIDE to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<VoidCheckUpdateRespose> {
		try {
			const response = await fetch('https://api.github.com/repos/Axenex/verit/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of veritIDE is available! Please reinstall (auto-updates are disabled on this OS).'
						action = 'reinstall'
					}
					else {
						message = 'veritIDE is up-to-date!'
					}
				}
				else {
